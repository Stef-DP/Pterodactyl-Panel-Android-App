...
@Composable
fun DeleteBackupPopup(
    activity: FragmentActivity,
    context: Context,
    state: ClientServerBackupsTabUiState,
    viewModel: ClientServerBackupsTabViewModel,
) {
    val backup = state.backups.find { it.attributes.uuid == state.backupToDelete }

    if (backup == null) return

    Popup(
        showPopup = state.backupToDelete != null,
        onDismissRequest = {
            viewModel.setBackupToDelete(null)
        },
    ) {
        Text(
            text = "Delete \"${backup.attributes.name}\"",
