...
private const val TAG = "ClientApi[updateServerStartupVariable]"
...
suspend fun updateServerStartupVariable(
    ...
    try {
        ...
        val requestBody = UpdateServerStartupVariableBody(
            key = key,
            value = value
        )

        val response = PterodactylApiClient.getClientApiService(serverUrl).updateServerStartupVariable(
