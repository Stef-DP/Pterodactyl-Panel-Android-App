...
private const val TAG = "ClientApi[listServerSchedules]"

suspend fun listServerSchedules(
    context: Context,
    serverId: String,
): Result<ListServerSchedulesResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).listServerSchedules(
        ...
        if (body is ListServerSchedulesResponse) {
