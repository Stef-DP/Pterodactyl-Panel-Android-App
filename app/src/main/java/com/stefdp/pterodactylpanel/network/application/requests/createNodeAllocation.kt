...
private const val TAG = "ApplicationApi[createNodeAllocation]"
...
suspend fun createNodeAllocation(
    ...
    try {
        ...
        val requestBody = CreateNodeAllocationBody(
            ip = ip,
            ports = ports,
            alias = alias
        )

        val response = PterodactylApiClient.getApplicationApiService(serverUrl).createNodeAllocation(
