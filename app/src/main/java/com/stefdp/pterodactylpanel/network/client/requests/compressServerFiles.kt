...
private const val TAG = "ClientApi[compressServerFiles]"
...
suspend fun compressServerFiles(
    ...
    try {
        ...
        val requestBody = CompressServerFilesBody(
            root = directory,
            files = files
        )

        val response = PterodactylApiClient.getClientApiService(serverUrl).compressServerFiles(
