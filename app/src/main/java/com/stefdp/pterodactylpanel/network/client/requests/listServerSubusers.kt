...
private const val TAG = "ClientApi[listServerSubusers]"

suspend fun listServerSubusers(
    context: Context,
    serverId: String,
): Result<ListServerSubusersResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).listServerSubusers(
        ...
        if (body is ListServerSubusersResponse) {
