...
private const val TAG = "ClientApi[rotateServerDatabasePassword]"

suspend fun rotateServerDatabasePassword(
    context: Context,
    serverId: String,
    databaseId: String,
): Result<ServerDatabase> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).rotateServerDatabasePassword(
