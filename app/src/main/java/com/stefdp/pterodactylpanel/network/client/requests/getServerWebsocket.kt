...
private const val TAG = "ClientApi[getServerWebsocket]"

suspend fun getServerWebsocket(
    context: Context,
    serverId: String,
): Result<GetServerWebsocketResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).getServerWebsocket(
        ...
        if (body is GetServerWebsocketResponse) {
