...
private const val TAG = "NodeApi[getNodeSystemV1]"

suspend fun getNodeSystemV1(
    nodeUrl: String,
    token: String,
): Result<GetNodeSystemV1Response> {
    try {
        val response = PterodactylApiClient.getNodeApiService(nodeUrl).getNodeSystemV1(
        ...
        if (body is GetNodeSystemV1Response) {
