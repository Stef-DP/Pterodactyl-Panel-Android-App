...
private const val TAG = "ClientApi[getServerStartupVariables]"

suspend fun getServerStartupVariables(
    context: Context,
    serverId: String,
): Result<GetServerStartupVariablesResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).getServerStartupVariables(
        ...
        if (body is GetServerStartupVariablesResponse) {
