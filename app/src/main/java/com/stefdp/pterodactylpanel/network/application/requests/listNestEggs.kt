...
private const val TAG = "ApplicationApi[listNestEggs]"

suspend fun listNestEggs(
    context: Context,
    nestId: Long,
    include: String? = null,
): Result<ListNestEggsResponse> {
    try {
        ...
        val response = PterodactylApiClient.getApplicationApiService(serverUrl).listNestEggs(
        ...
        if (body is ListNestEggsResponse) {
