...
private const val TAG = "ClientApi[deleteServerFiles]"
...
suspend fun deleteServerFiles(
    ...
    try {
        ...
        val requestBody = DeleteServerFilesBody(
            files = files,
            root = directory
        )

        val response = PterodactylApiClient.getClientApiService(serverUrl).deleteServerFiles(
