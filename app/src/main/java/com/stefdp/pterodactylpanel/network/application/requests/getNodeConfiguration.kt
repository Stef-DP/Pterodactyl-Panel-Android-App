...
private const val TAG = "ApplicationApi[getNodeConfiguration]"

suspend fun getNodeConfiguration(
    context: Context,
    nodeId: Long,
): Result<GetNodeConfigurationResponse> {
    try {
        ...
        val response = PterodactylApiClient.getApplicationApiService(serverUrl).getNodeConfiguration(
        ...
        if (body is GetNodeConfigurationResponse) {
