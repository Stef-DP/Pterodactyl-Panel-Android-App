...
private const val TAG = "ClientApi[getServerActivity]"

suspend fun getServerActivity(
    context: Context,
    serverId: String,
    filterEvent: String? = null,
    sort: GetServerActivityQuerySort? = null,
    include: String? = null,
    page: Long? = null,
    perPage: Int? = null
): Result<GetServerActivityResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).getServerActivity(
        ...
        if (body is GetServerActivityResponse) {
