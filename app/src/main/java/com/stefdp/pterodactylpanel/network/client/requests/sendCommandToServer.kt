...
private const val TAG = "ClientApi[sendCommandToServer]"
...
suspend fun sendCommandToServer(
    ...
    try {
        ...
        val requestBody = SendCommandToServerBody(
            command = command
        )

        val response = PterodactylApiClient.getClientApiService(serverUrl).sendCommandToServer(
