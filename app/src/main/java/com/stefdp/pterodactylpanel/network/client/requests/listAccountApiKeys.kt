...
private const val TAG = "ClientApi[listAccountApiKeys]"

suspend fun listAccountApiKeys(
    context: Context,
): Result<ListAccountApiKeysResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).listAccountApiKeys(
        ...
        if (body is ListAccountApiKeysResponse) {
