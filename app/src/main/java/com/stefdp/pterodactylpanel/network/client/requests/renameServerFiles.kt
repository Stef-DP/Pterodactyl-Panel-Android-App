...
private const val TAG = "ClientApi[renameServerFiles]"

suspend fun renameServerFiles(
    context: Context,
    serverId: String,
    directory: String,
    files: List<RenameServerFilesBody.File>
): Result<Unit> {
    try {
        ...
        val requestBody = RenameServerFilesBody(
            root = directory,
            files = files
        )

        val response = PterodactylApiClient.getClientApiService(serverUrl).renameServerFiles(
