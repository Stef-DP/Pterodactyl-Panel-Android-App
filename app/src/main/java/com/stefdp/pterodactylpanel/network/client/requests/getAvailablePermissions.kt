...
private const val TAG = "ClientApi[getAvailablePermissions]"

suspend fun getAvailablePermissions(
    context: Context,
): Result<GetAvailablePermissionsResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).getAvailablePermissions(
        ...
        if (body is GetAvailablePermissionsResponse) {
