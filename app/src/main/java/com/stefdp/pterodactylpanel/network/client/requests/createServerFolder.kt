...
private const val TAG = "ClientApi[createServerFolder]"
...
suspend fun createServerFolder(
    ...
    try {
        ...
        val requestBody = CreateServerFolderBody(
            name = name,
            root = directory
        )

        val response = PterodactylApiClient.getClientApiService(serverUrl).createServerFolder(
