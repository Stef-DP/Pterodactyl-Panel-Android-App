...
private const val TAG = "ClientApi[listAccountSshKeys]"

suspend fun listAccountSshKeys(
    context: Context,
): Result<GetAccountSshKeysResponse> {
    try {
        ...
        val response = PterodactylApiClient.getClientApiService(serverUrl).listAccountSshKeys(
        ...
        if (body is GetAccountSshKeysResponse) {
