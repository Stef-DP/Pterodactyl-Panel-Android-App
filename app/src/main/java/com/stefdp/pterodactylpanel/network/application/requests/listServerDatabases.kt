...
private const val TAG = "ApplicationApi[listServerDatabases]"

suspend fun listServerDatabases(
    context: Context,
    serverId: Long,
    include: String? = null,
): Result<ListServerDatabasesResponse> {
    try {
        ...
        val response = PterodactylApiClient.getApplicationApiService(serverUrl).listServerDatabases(
        ...
        if (body is ListServerDatabasesResponse) {
