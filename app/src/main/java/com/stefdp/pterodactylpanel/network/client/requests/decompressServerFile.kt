...
private const val TAG = "ClientApi[decompressServerFile]"
...
suspend fun decompressServerFile(
    ...
    try {
        ...
        val requestBody = DecompressServerFileBody(
            file = file,
            root = directory
        )

        val response = PterodactylApiClient.getClientApiService(serverUrl).decompressServerFile(
